...
b_values = [f'b2{i}' for i in range(1, 6)]  # 生成b11, b12, ..., b17
i_values = [str(i) for i in range(1, 8)]  # 将数字转换为字符串，因为URL中可能需要字符串  
...
for b in b_values:  
    for month in months: 
        for i in i_values: 
            url = f"https://www.maiche.com/rank/sale/{b}d{month}p{i}.html"  
            urls.append(url)  
# 创建webdriver实例（在循环外）  
    ...
    for url in urls:
        ...
        for new in news:       
            try:
                    ...
                    #print(price_range)  
                    # 全国销量  
                    sales_volume = new.find('div',class_='item fr sale').find('span', class_='item-num').get_text().strip()  
                    ...
                    print(cartime,cartype,carname,price_range,sales_volume,quality_guarantee,environment_standard,soil_consumption,fuel_label)
                    # 列的名称，即表格的表头  
                    header = ['时间', '车型', '车名', '厂商指导价格范围', '全国月销量', '质保', '环保标准', '油耗', '燃油标号']  
                    listt = [cartime, cartype, carname, price_range, sales_volume, quality_guarantee, environment_standard, soil_consumption, fuel_label]  

                    if not os.path.exists("车辆销售排行信息.csv") or os.path.getsize("车辆销售排行信息.csv") == 0:  
                        # 文件不存在或为空，写入表头  
                        with open("车辆销售排行信息.csv", "w", newline='', encoding="utf-8-sig") as f:  
                            write = csv.writer(f)  
                            write.writerow(header)
                    # 写入CSV文件，包括表头（如果文件不存在或为空）  
                    with open("车辆销售排行信息.csv", "a+",newline='',encoding="utf-8-sig") as f:  
                        write=csv.writer(f)
