...
# 定义需要查询的页码列表  
b_values = [f'{i}' for i in range(422, 847)]  # 生成b11, b12, ..., b17

urls = []  
for b in b_values:  
    url = f"http://www.moe.gov.cn/was5/web/search?channelid=239993&page={b}"  
    urls.append(url)  
# 创建webdriver实例（在循环外）  
...
try:  
    for url in urls:
        ...
        # driver.maximize_window()  # 这行代码不是必要的，取决于需求  
        content = driver.page_source  
        #print(content)  
        soup = BeautifulSoup(content, 'lxml')  
        alist = soup.find('div',class_="scy_lbsj-right-nr").find('ul')  
        # print(alist) 
        news=alist.find_all('li')  
        #print(len(news))  
        #类别
        category=driver.find_element(By.XPATH, '//*[@id="1087301674"]/a').text.strip()
        #print(category)
...
#进入二级链接http://www.moe.gov.cn/jyb_xxgk/moe_1777/moe_1778/202402/t20240229_1117317.html
                ...
                soup=BeautifulSoup(content,'lxml')
#标题
                title=soup.find('div',id='downloadContent').find('h1').get_text().strip()
                #print(title)
#发布单位       
                try:
                    release_unit=soup.find('p',class_='moe-policy-wenhao').get_text().strip()
                except:
...
#政策内容       
                policy_content = ""  
                content=soup.find('div',id='downloadContent').find_all('p')
