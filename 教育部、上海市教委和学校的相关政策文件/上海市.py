...
#来源
hea="上海教育委员会"
for lit in littles[20:]:
    try:
        #大类
        category="政策文件"
        #中类
        midcategory=lit.find('span',class_='node_name').get_text().strip()
        print(midcategory)
        twourl=lit.find('a',class_='level1 ariaskiptheme').get('href')
        print(twourl)
        #二级网页
        newtwourl='https://edu.sh.gov.cn'+twourl
        print(newtwourl)
        driver.get(newtwourl)
        time.sleep(1)
        content=driver.page_source
        soup=BeautifulSoup(content,'lxml')
        seclistss=soup.find('ul',class_='level1 line')
